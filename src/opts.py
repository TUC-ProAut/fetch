...
from backbones import ALL_BACKBONES
from samplers import ALL_SAMPLERS

def parse_general_args(args=None) -> argparse.Namespace:
...
def add_general_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument('--data_dir', type=str, default='/daten/marwei/pytorch', help='Directory where all datasets are stored')
    parser.add_argument('--log_dir', type=str, default='/home/marwei/code/EncodedGDumb/logs', help='Directory where all logs are stored')
    parser.add_argument('--dataset', type=str, required=True, help='Name of dataset', choices=ALL_DATASETS)
    ...
    parser.add_argument('--regularization', type=str, default='cutmix', choices=['none', 'cutmix'], help='Regularization types')
    ...
    parser.add_argument('--whole_dataset', action="store_true", help='Trains on the whole dataset without using the memory') 

    parser.add_argument('--encoder', type=str, required=True, help='name of encoder', choices=ALL_ENCODERS) 
    parser.add_argument('--compressor', type=str, required=True, help='name of compressor', choices=ALL_COMPRESSORS) 
    parser.add_argument('--backbone', type=str, required=True, help='name of backbone network', choices=ALL_BACKBONES)
    parser.add_argument('--sampler', type=str, default='greedy_sampler', help='sampling strategy', choices=ALL_SAMPLERS)
    ...
    parser.add_argument('--device', default='cuda', choices=['cuda', 'cpu', 'mps'], help='torch-device') 
