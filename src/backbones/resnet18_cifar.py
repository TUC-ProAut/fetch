...
class Resnet18_CIFAR(nn.Module):
    def __init__(self, opt: Namespace, dataset_info: ContinualDatasetInfo, compressor_info: CompressorDecompressorInfo):
        """resnet 18, pretrained on the first two CIFAR10 classes. It is possible to cut off layers form the front."""
        super().__init__()
        full_resnet = resnet18()
        full_resnet.fc = torch.nn.Linear(512, 10)
        full_resnet.load_state_dict(torch.load('src/weights/resnet18_cifar10_classes01_e110.pt'))
